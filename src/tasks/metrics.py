...
from sklearn.metrics import f1_score, roc_auc_score, matthews_corrcoef, confusion_matrix, roc_curve, precision_recall_curve, auc
...
def mcc(logits, y):
    ...
    filtered_y = y[mask]
    y_hat = torch.argmax(filtered_logits, dim=-1)
    # y_hat = torch.argmax(logits, dim=-1)
    return matthews_corrcoef(filtered_y.cpu().numpy(), y_hat.cpu().numpy())

def multi_class_mcc(logits, y):
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    y_hat = torch.argmax(logits, dim=-1)
    ...
    for k in range(n_classes):
        ...
        mcc_denominator = np.sqrt(sum_tk * sum_pk * (sum_ck + fn) * (sum_ck + fp))
        mcc = mcc_numerator / mcc_denominator if mcc_denominator != 0 else 0
        
    # 多分类情况下，MCC的平均
    mcc_avg = mcc / n_classes
    return mcc_avg
...
def accuracy(logits, y):
    ...
    accu = torch.eq(preds_masked, y_masked).float().mean()
    # print(f"accu= {accu}")
    return torch.eq(preds_masked, y_masked).float().mean()
...
def f1_macro(logits, y, prefix='test'):
    logits = logits.view(-1, logits.shape[-1])
    b = y.shape[0]
    y = y.view(-1)
    ...
    preds, labels = filtered_logits.detach().cpu(), filtered_y.detach().cpu()

    # test_set_size = 2001   # solubility
    # test_set_size = 337   # cleavage
    # test_set_size = 4152   # signalp
    test_set_size = 513 * 512   # signalp

    if not hasattr(f1_macro, "all_preds"):
        f1_macro.all_preds = []
        f1_macro.all_labels = []
        f1_macro.total_length = 0 

    if prefix == 'test':
        if not hasattr(f1_macro, "epoch"):
            f1_macro.epoch = 0
        f1_macro.all_preds.append(preds)
        f1_macro.all_labels.append(labels)
        # f1_macro.total_length += labels.numel()
        f1_macro.total_length += b * 512


        if f1_macro.total_length >= test_set_size:
            all_preds = torch.cat(f1_macro.all_preds)
            all_labels = torch.cat(f1_macro.all_labels)

            base_path = '/home/lr/zym/research/hyena-dna/results/prot-base/disorder'
            os.makedirs(base_path, exist_ok=True)
            file_name = f"disorder_preds_labels_epoch_{f1_macro.epoch}.csv"
            path = os.path.join(base_path, file_name)


            np.savetxt(path, np.column_stack((all_preds.numpy(), all_labels.numpy())), delimiter=",", header="Prediction,Label", comments='')
            f1_macro.epoch += 1
    ...
    y_hat = torch.argmax(filtered_logits, dim=-1)
    # y_hat = torch.argmax(logits, dim=-1)
    return f1_score(filtered_y.cpu().numpy(), y_hat.cpu().numpy(), average="macro")
...
def f1_per_class(logits, y):
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    y_hat = torch.argmax(logits, dim=-1)
    f1_scores = f1_score(y.cpu().numpy(), y_hat.cpu().numpy(), average=None)
    print(f1_scores)
    return f1_scores

# def precision_per_class(logits, y):
...
def roc_auc_macro(logits, y):
    ...
    # print(f'y_true: {y_true_filtered} \n y_score: {y_score_filtered}')
    
    if len(np.unique(y_true_filtered)) < 2:
        return 0
    
    return roc_auc_score(
        y_true_filtered, y_score_filtered[:, 1], average="macro", multi_class="ovr"
    )
    ...
    with open(filename, 'a') as file: 
        for key, value in metrics.items():
            data_to_write = {
                ...
                'roc_thresholds': value['roc_thresholds'].tolist(),
                'precision': value['precision'].tolist(),
                'recall': value['recall'].tolist(),
                'pr_thresholds': value['pr_thresholds'].tolist()
            }
            file.write(json.dumps(data_to_write) + '\n')  # 每个类别的metrics为一行json字符串



def safe_roc_auc_score(logits, y):
...
def spearman_rho(outs, y, prefix='test', len_batch=None):
    preds, labels = outs.detach().cpu(), y.detach().cpu()

    test_set_size = 12851   # stability
    # test_set_size = 27217   # fluorescence
    if not hasattr(spearman_rho, "all_preds"):
        spearman_rho.all_preds = []
        spearman_rho.all_labels = []
        spearman_rho.total_length = 0 

    if prefix == 'test':
        if not hasattr(spearman_rho, "epoch"):
            spearman_rho.epoch = 0
        spearman_rho.all_preds.append(preds)
        spearman_rho.all_labels.append(labels)
        spearman_rho.total_length += preds.numel()

        if spearman_rho.total_length >= test_set_size:
            # 将累积的 preds 和 labels 拼接成单个张量
            all_preds = torch.cat(spearman_rho.all_preds)
            all_labels = torch.cat(spearman_rho.all_labels)

            base_path = '/home/lr/zym/research/hyena-dna/results/prot-base/stability'
            os.makedirs(base_path, exist_ok=True)
            file_name = f"stability_preds_labels_epoch_{spearman_rho.epoch}.csv"
            path = os.path.join(base_path, file_name)


            np.savetxt(path, np.column_stack((all_preds.numpy(), all_labels.numpy())), delimiter=",", header="preds,labels", comments='')
            spearman_rho.epoch += 1

            spearman_rho.all_preds = []
            spearman_rho.all_labels = []
            spearman_rho.total_length = 0
            
    # np.savetxt(path, np.column_stack((preds.numpy(), labels.numpy())), delimiter=",", header="preds,labels", comments='')
    spearman_corr, _ = stats.spearmanr(preds, labels)
    # print('spearman epoch: ', spearman_rho.epoch)
    # print(f"spearman_corr: {spearman_corr}, p value: {_}")
