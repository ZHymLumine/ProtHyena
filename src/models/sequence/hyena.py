...
class HyenaFilter(OptimModule):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"d_model: {d_model}, emb_dim: {emb_dim}, order: {order}, num_inner_mlps: {num_inner_mlps}")
...
class HyenaOperator(nn.Module):
    def __init__(
        ...
        **filter_args,
        ...
        # print(f"block_dim : {block_dim}, num_blocks: {num_blocks} ")
    ...
    def forward(self, u, *args, **kwargs):
        ...
        *x, v = uc.split(self.d_model, dim=2)
        ...
        # print("v shape: ", v.shape)    # x[0], x[1], v shpae (b, 1, d_model, 1, length)
        k = self.filter_fn.filter(l_filter) # get hyena filters (h_1 ... h_n)

        # print("k shape before arrange: ", k.shape) (1, 512, 256)
        # `c` is always 1 by default
        k = rearrange(k, "c l (v o) -> c o v l", v=self.head_dim, o=self.order - 1)[0]
        # print("k shape after arrange: ", k.shape)  (1, 256, 512)
        bias = rearrange(
        ...
        for o, x_i in enumerate(reversed(x[1:])):
            ...
            # print("v in for loop : ", v.shape)
            # Long Convolution
            # the bias term is broadcasted. Last dimension (l) is handled by fftconv
            
            v = self.filter_fn(v, l_filter, k=k[o], bias=bias[o, None, :, None]) # k: hyena filter
            # print(o, " ", k[o].shape) (d_model, l)
            ...
            # print(v.shape) # (b, 1, d_model, 1, length)
                
        # new_v = v.squeeze(1).squeeze(2) # (b, d_model, l)
        # new_x0 = x[0].squeeze(1).squeeze(2) # (b, d_model, l)
        # # print(new_v.shape, " ", new_x0.shape)
        # new_v = new_v.transpose(1, 2)
        # hyena_matrix = torch.bmm(new_v, new_x0)
        # print("H(u): ", hyena_matrix.shape)

        # hyena_matrix = hyena_matrix.detach().cpu()
        # matrix_abs = np.abs(hyena_matrix)
        # plt.figure(figsize=(10, 10))  # 设置图像大小
        # plt.imshow(matrix_abs[0], cmap='Oranges')  # 绘制热图
        # # plt.colorbar()  # 显示颜色条
        # plt.axis('off')
        # plt.title("Hyena matrix.png")
        # plt.xlabel("X-axis")
        # plt.ylabel("Y-axis")

        # # 保存图像
        # plt.savefig('matrix_visualization.png', dpi=300)  # 指定文件名和分辨率
